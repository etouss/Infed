...
WHERE NOT EXISTS(
  ...
  WHERE l_orderkey = o_orderkey
  AND (l_partkey = p_partkey)
  AND (l_suppkey = s_suppkey IS NOT FALSE)
  AND (s_nationkey = n_nationkey IS NOT FALSE)
  AND (p_name LIKE '%floral%' IS NOT FALSE)
  AND n_name = 'FRANCE' IS NOT FALSE
);
...
WHERE NOT EXISTS(
  ...
  FROM lineitem,part,supplier,nation
  WHERE l_orderkey = o_orderkey
  AND (l_partkey = p_partkey OR l_partkey IS NOT DISTINCT FROM NULL)
  AND (l_suppkey = s_suppkey OR l_suppkey IS NOT DISTINCT FROM NULL)
  AND (s_nationkey = n_nationkey OR s_nationkey IS NOT DISTINCT FROM NULL)
  AND (p_name LIKE '%floral%' OR p_name IS NOT DISTINCT FROM NULL)
  AND n_name = 'FRANCE'
);
